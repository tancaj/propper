...
namespace pr
{
	namespace detail
	{
		...
		{
		template<typename _type> struct is_vector : public std::false_type {};
		...
		template<typename _type>
		concept propped_object = std::is_class_v<std::decay_t<_type>> 
		...
		template<typename _type>
		concept json_trivials = std::is_same_v<std::decay_t<_type>, int>
		...
		concept predicate_invocable_types = std::predicate<_type, int> 
			...
			|| std::predicate<_type, bool>
			|| std::predicate<_type, const char*>
			|| std::predicate<_type, std::string>;

		template<typename _type>
		concept predicate = requires(_type pred)
		{
			{pred.get_error()} -> std::same_as<std::string>;
		}
