...
namespace pr
{
	namespace detail
	{
		...
		template<typename _type>
		concept json_types = std::is_same_v<std::decay_t<_type>, int>
		...
		template<typename _type>
		concept property_type = property_object<_type> || json_types<_type> || is_vector<_type>::value;
		...
		template<typename _type>
		concept predicate_error_string = std::is_same_v<std::decay_t<_type>, std::string>;
